...
from favorite_listing import FavoriteListing
from parser_feedback import ParserFeedback
...
def find_all_users_favored_listing_in_shop(shop_id):
    ...
    for listing_batch in listing.find_all_shop_listings_active(shop_id):
        for listing in listing_batch:
            users = favorite_listing.find_all_listing_favored_by(str(listing))
...
def connect_all_users_who_left_feedback(user_id, shop_id):
    ...
    for batch_aimed_users in fp.get_users_left_feedback_to_shop(shop_id):
        # only users who're not yet connected.
