...
class ParserFeedback:
    ...
    def gen_pages_for_shop(self, shop_id):
        """
        ...
        """
        for i in range(1, 10):
            uri = ''.join((self.url_shop, shop_id,
    ...
    def get_users_left_feedback_to_shop(self, shop_id):
        """
        ...
        users = set()
        for url in self.gen_pages_for_shop(shop_id):
            et.info(msg='send request to ' + url)
            r = requests.get(url)
            data = r.content.decode('utf-8')
            new_users = self.get_users_from_xml(data)
...
def main():
    fp = ParserFeedback()
    users = fp.get_users_left_feedback_to_shop('OrgonitePyramid')
    for user in users:
        print(user)
